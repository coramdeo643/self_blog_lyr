...
public class ArticleService {
	...
	// 게시글 사진 업로드 서비스
	@Transactional
	public Article uploadPhotoImage(Long articleId, MultipartFile multipartFile) {
		Article article = findById(articleId);
		String oldImagePath = article.getPictureImagePath();
		try {
			String newImagePath = photoUploadService.uploadPhotoImage(multipartFile);
			System.out.println("uploaded");
			if (oldImagePath != null) {
				photoUploadService.deletePhotoImage(oldImagePath);
				System.out.println("deleted");
			}
			article.setPictureImagePath(newImagePath);
			System.out.println("set");
			return article;
		} catch (IOException e) {
			throw new Exception400("Photo image upload failed");
		}
	}

	@Transactional
	public Article deletePhotoImage(Long articleId) {
		Article article = findById(articleId);
		String imagePath = article.getPictureImagePath();
		article.setPictureImagePath(null);
		if (imagePath != null && imagePath.isEmpty() == false) {
			photoUploadService.deletePhotoImage(imagePath);
		}
		return article;
	}
